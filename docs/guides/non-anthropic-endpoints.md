...
Set whichever your provider documents — Kimi's published 1M configuration sets both. Pair either with `CLAUDE_CODE_AUTO_COMPACT_WINDOW` so compaction fires against the real ceiling — [Tune auto-compaction](./auto-compact.md) covers the levers, their precedence, and the cost trade of setting it deliberately lower.
